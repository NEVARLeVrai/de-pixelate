...
for name, frame in load_png_frames('frames'):
  ...
  #print(ff.size(), sf1.size(), sf2.size())
  ...
  image = torch.cat((
    ...
    imageB,
    imageC,
    torch.full_like(imageA, 0xFF)
  ), 0)
  #transform(image).save(os.path.join('frames_detected', name))
  ...
  if foundX and foundY:
    ...
    continue
  image[0, y, x] = 0xFF #set the red pixel for later stateg
  image[1, y, x] = 0
  image[2, y, x] = 0
  image[3, y, x] = 0xFF
